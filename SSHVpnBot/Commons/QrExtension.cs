...
public static class QrCodeBitmapExtensions
{
    ...
    {
        // check arguments
        if (scale <= 0) throw new ArgumentOutOfRangeException(nameof(scale), "Value out of range");
        if (border < 0) throw new ArgumentOutOfRangeException(nameof(border), "Value out of range");

        var size = qrCode.Size;
        var dim = (size + border * 2) * scale;

        if (dim > short.MaxValue) throw new ArgumentOutOfRangeException(nameof(scale), "Scale or border too large");
